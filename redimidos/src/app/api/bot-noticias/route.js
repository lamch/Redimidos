...
async function procesarConClaude(articulos, cantidad) {
  ...
  const msg = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Eres el editor principal de Redimidos.net, un portal de noticias cristianas para América Latina.
...
async function getImagen(query) {
  ...
  if (!key) {
    // Imagen por defecto temática cristiana si no hay key
    const defaults = [
...
export async function POST(request) {
  ...
  try {
    ...
    const generadas = await procesarConClaude(todas, cantidad);

    // 3. Obtener imágenes y construir objetos con formato de noticias.json
    const nuevas = [];
    ...
    // 4. En dry-run, solo devolver las noticias sin escribir
    if (dryRun) {
      return NextResponse.json({ ok: true, dryRun: true, noticias: nuevas });
    }

    // 5. Devolver las noticias para que el GitHub Action las escriba al JSON
    // (Vercel es read-only en producción, el write lo hace el Action via git)
